...
saturation/lightness on every country colour [DB `c` + `seededColor` fallback]. Flows
to the server via the join `geoColors` so map screenshots match.) (v108 = hover-tooltip `#tip` now
