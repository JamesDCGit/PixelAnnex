...
const SERVER_VERSION       = '2026-08-31-v189';
...
const httpServer = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/robots.txt') {
    ...
    // v179: /shots/ and /timelapse/ are now ALLOWED — they hold the world snapshots and
    // timelapse GIFs that the share pages use as og:image. Blocking them meant Google
    // could not fetch our own social/rich-result images (and Search Console flagged
    // them as "Blocked by robots.txt"). They are public media with nothing sensitive.
    // Still blocked: /admin (operator dashboard), /api/ (JSON endpoints, no SEO value),
    // /auth/ (OAuth redirects — these were the "Page with redirect" reports).
  ...
  if (url.pathname === '/sitemap.xml') {
    ...
    res.end('<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n' +
      ['/', '/about', '/how-to-play', '/strategy', '/guides', '/guides/encirclement',
       '/guides/conquest', '/guides/pixels-and-regeneration', '/guides/winning-a-war',
       '/guides/the-living-world', '/faq', '/privacy', '/terms'].map(p =>
  ...
  function _seoPage(title, desc, bodyHtml, canonicalPath) {
    return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">' +
      ...
      '<p style="font-size:11px;color:#475569">Created by <a href="https://www.prettyneatpixels.com">Pretty Neat Pixels</a> · <a href="/">Play</a> · <a href="/about">About</a> · <a href="/how-to-play">How to play</a> · <a href="/strategy">Strategy</a> · <a href="/guides">Guides</a> · <a href="/faq">FAQ</a> · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a> · <a href="https://discord.gg/UHQRqXDpBE">Discord</a> · <a href="https://x.com/PixelAnnexGame">X</a></p>' +
      '</body></html>';
  }


  // v190: guides content section — five hand-written articles + an index.
  // Written individually rather than generated per country/war on purpose:
  // templated pages at scale are exactly the thin-content pattern removed
  // from /share/war-N in v189a.
  if (url.pathname === '/guides') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('PixelAnnex Guides — Mechanics, Tactics and Strategy', 'In-depth PixelAnnex guides: encirclement maths, how conquest thresholds really work, the regeneration and underdog system, endgame tactics, and the living world.',
      '<h1>PixelAnnex guides</h1><p>Detailed explanations of how PixelAnnex actually works underneath &mdash; the mechanics, the numbers behind them, and how experienced players use them. If you are completely new, read <a href="/how-to-play">how to play</a> first, then come back.</p><h2><a href="/guides/encirclement">The encirclement guide</a></h2><p>The most efficient action in the game, why a ring costs a tenth of what filling does, the four ways a capture pays out, and why breaking an enemy loop beats repainting lost ground.</p><h2><a href="/guides/conquest">How conquest actually works</a></h2><p>The two separate routes a nation can fall by, why big countries fall differently from small ones, how holding outposts armours your homeland, and what happens to a country after it falls.</p><h2><a href="/guides/pixels-and-regeneration">Pixels, regeneration and the underdog system</a></h2><p>Where your pixels come from, why small nations refill faster than superpowers, and the four situations that change your rate.</p><h2><a href="/guides/winning-a-war">Winning a war</a></h2><p>The 65/65 bloc condition, why alliances win together, what sudden death changes, and why conquest scoring makes late aggression pay.</p><h2><a href="/guides/the-living-world">The living world</a></h2><p>The 166 AI nations that fight while you are offline, roaming monsters, the real-time day/night cycle, and the 250 collectables.</p>', '/guides'));
    return;
  }
  if (url.pathname === '/guides/encirclement') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('Encirclement Guide — PixelAnnex', 'How encirclement works in PixelAnnex: the maths that make a ring cheaper than filling, what it pays out, allied walls, and how to break an enemy ring before it closes.',
      '<h1>The encirclement guide</h1><p>Encirclement is the single most efficient action in PixelAnnex, and the mechanic that separates players who climb the leaderboard from players who spend all day painting. If you learn one thing about this game, learn this.</p><h2>The maths</h2><p>Your pixel bucket refills slowly, so the only question that matters is how much territory each pixel you spend actually buys you. Filling an area costs one pixel per pixel: colouring a 40&times;40 patch costs 1,600 from your bucket. Drawing a <em>ring</em> around that same patch costs roughly 160 &mdash; the perimeter only &mdash; and everything sealed inside flips to you the moment the loop closes.</p><p>That is a tenfold difference on a modest square, and it gets better as the enclosed area grows, because perimeter scales linearly while area scales quadratically. Doubling the size of your ring doubles its cost but quadruples the prize.</p><h2>What it pays</h2><p>A successful encirclement pays out four separate ways:</p><ul><li><strong>The territory itself</strong> &mdash; every unclaimed or enemy pixel inside the loop becomes yours instantly.</li><li><strong>Double points</strong> &mdash; encircled pixels score twice what painted pixels do.</li><li><strong>A pixel refund</strong> &mdash; roughly a tenth of the captured area comes straight back into your bucket, which often funds the next ring.</li><li><strong>A regeneration surge</strong> &mdash; your refill rate jumps for about a minute afterwards, scaling with how much you captured.</li></ul><p>The refund and the surge are what make encirclements <em>chain</em>. Close one ring, and you are frequently holding enough pixels and enough regeneration to immediately close another. Players who understand this play in bursts rather than a constant trickle.</p><h2>The rules</h2><p>A loop has to enclose at least 15 pixels to count, which stops one-pixel accidents from triggering the bonus. There is also an upper bound on a single capture, so a ring drawn around half a continent will not resolve &mdash; if a huge enclosure fails to fire, that is why, and the fix is several medium rings rather than one enormous one.</p><p>Crucially, <strong>your ally\'s borders count as your wall</strong>. You do not need to draw the whole loop yourself: a line of allied territory, or a coastline, completes the circuit just as well as your own pixels. Some of the largest captures in the game are made by closing a short gap between two existing borders rather than drawing anything resembling a circle.</p><h2>Defending against it</h2><p>Because encirclement is so strong, the highest-value defensive action in the game is not repainting lost ground &mdash; it is <strong>breaking rings before they close</strong>.</p><p>A single one of your pixels sitting in the path of an enemy loop invalidates the entire capture. If you watch an attacker spend two minutes drawing an arc through your territory, one well-placed pixel wastes all of it. Scan your borders for half-drawn curves rather than for missing patches; the curve is the threat.</p><p>This is also why blanket repainting is usually a poor use of a bucket. Ten pixels spent plugging three potential rings will save more territory than a hundred spent colouring in an area nobody is trying to enclose.</p><p>Next: <a href="/guides/conquest">how conquest thresholds actually work</a>.</p>', '/guides/encirclement'));
    return;
  }
  if (url.pathname === '/guides/conquest') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('How Conquest Works — PixelAnnex', 'The two routes a country can fall by in PixelAnnex, why big countries fall differently from small ones, how holding outposts defends your homeland, and what happens to a nation after it falls.',
      '<h1>How conquest actually works</h1><p>&ldquo;Hold 70% and it falls&rdquo; is the short version, and it is true often enough to be useful &mdash; but it is not the whole rule, and the difference matters enormously when you are deciding where to spend a week of pixels.</p><h2>Route one: the champion</h2><p>The straightforward path. One country &mdash; counting its allies\' pixels alongside its own &mdash; holds roughly 70&ndash;75% of a nation\'s land, and that nation falls to it.</p><p>The exact share required is not fixed. Smaller countries demand a <em>higher</em> percentage, which is deliberate: without it, a fifty-pixel micro-nation would fall to a stray brush stroke. If you are attacking somewhere tiny, expect to need almost all of it.</p><h2>Route two: the contested fall</h2><p>The champion rule alone produces a problem on very large countries. Somewhere the size of China or Russia has enormous interior nobody ever paints, so a measurement against <em>total</em> land means the country is effectively unconquerable no matter how thoroughly its populated edges are carved up.</p><p>So for large nations there is a second route, measured against <strong>painted</strong> territory rather than total area. When foreign invaders collectively dominate the part of the country that anyone has actually contested, it falls to whichever single attacker holds the most &mdash; even if that attacker is below the champion bar. There is still a floor: the leading attacker must hold at least half the whole country, so a country cannot be handed to someone with a token presence.</p><p>The practical consequence is that <strong>a country already carved up by several invaders is far closer to falling than an untouched one</strong>. Finishing someone else\'s war is usually cheaper than starting your own.</p><h2>Empires defend themselves</h2><p>Every conquered outpost a nation holds raises the threshold needed to take its <em>homeland</em>, up to a ceiling. A sprawling empire is genuinely harder to decapitate than an isolated country of the same size.</p><p>This cuts both ways. If you are struggling to take a well-defended homeland, stripping its outposts first lowers the bar on the capital. If you are defending, holding outposts is not just points &mdash; it is armour.</p><h2>Read the hover panel</h2><p>Every country in the game displays exactly how far it is from falling when you hover it, including who holds what share. That number already accounts for the country\'s size and its empire bonus, so it is the real figure rather than a rule of thumb. Checking it before committing is the single easiest habit to build, and it is the difference between a two-day siege that succeeds and one that stalls at 60%.</p><h2>After the fall</h2><p>A newly conquered country is briefly immune, so it cannot be flipped again in the same minute by a rival who happened to be one pixel behind. After that it behaves like any other territory: invaders can chip away and it will transfer to a new dominant holder. The original nation does not get it back automatically.</p><p>If a country loses its homeland but still holds conquered outposts elsewhere, it is <strong>exiled</strong> rather than destroyed &mdash; it fights on at reduced regeneration until it reclaims its capital. Lose the outposts too and it is gone for the round, its remaining territory released as neutral <em>fallen</em> land that anyone can claim.</p><p>Next: <a href="/guides/pixels-and-regeneration">where your pixels actually come from</a>.</p>', '/guides/conquest'));
    return;
  }
  if (url.pathname === '/guides/pixels-and-regeneration') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('Pixels, Regeneration and the Underdog System — PixelAnnex', 'How the pixel bucket refills in PixelAnnex, why small nations regenerate faster than superpowers, and the four situations that change your rate: encirclement, exile, fight-back and sudden death.',
      '<h1>Pixels, regeneration and the underdog system</h1><p>Everything you do in PixelAnnex is paid for out of one resource. Understanding how that resource replenishes is what turns the game from clicking into strategy.</p><h2>The bucket</h2><p>You hold a bucket of pixels. Painting one pixel costs one. The bucket refills continuously over time, and its <em>capacity</em> &mdash; the ceiling it fills to &mdash; rises with your rank, from 50 at Soldier to 100 at Admiral.</p><p>Note that rank raises your capacity, not your rate. A higher rank lets you bank a bigger burst for one decisive push; it does not make you passively richer.</p><h2>The underdog curve</h2><p>Refill is measured as an absolute rate in pixels per second, and it is <strong>scaled to how much of the world you hold</strong>. A tiny nation regenerates several times faster than a superpower &mdash; the smallest countries approach the hard ceiling on size alone, while the largest sit near the baseline.</p><p>This is the single most important balance decision in the game, and it has a real consequence for how you play: <strong>being small is not a losing position</strong>. A micro-nation with a fast trickle can out-paint a giant over a day. It is also why conquering everything in sight is not automatically correct &mdash; as your holdings grow, your underdog bonus fades, and a sprawling empire refills slowly.</p><h2>The four modifiers</h2><ul><li><strong>Encirclement</strong> &mdash; closing a loop adds a temporary surge on top of your base rate for about a minute. See <a href="/guides/encirclement">the encirclement guide</a>.</li><li><strong>Fight-back</strong> &mdash; while your homeland is being overrun, your rate is pushed to maximum. Being invaded makes you dangerous, and a defender who keeps their nerve often has more pixels per minute than the attacker.</li><li><strong>Exile</strong> &mdash; if your homeland falls but your outposts hold, your base rate is cut until you reclaim your capital. Exile is survivable but it is meant to hurt.</li><li><strong>Sudden death</strong> &mdash; once only a handful of nations remain standing, <em>everyone\'s</em> rate doubles. See <a href="/guides/winning-a-war">the endgame guide</a>.</li></ul><h2>Spending well</h2><p>Because the rate is slow and the ceiling is low, the players who do best are not the ones online longest &mdash; they are the ones who never spend a full bucket on something a ring would have taken. Two habits are worth building:</p><p>First, <strong>never fill an interior</strong>. If you find yourself colouring the middle of a shape, stop and draw the outline instead. Second, <strong>bank before you strike</strong>. A full bucket spent on one closed loop is worth several times the same pixels dribbled out as they arrive, because the refund and surge from a successful encirclement compound into the next action.</p><p>Treasure chests appear on the map periodically and hand the first player to click one a lump of pixels and points outright. They are worth interrupting whatever you are doing for.</p>', '/guides/pixels-and-regeneration'));
    return;
  }
  if (url.pathname === '/guides/winning-a-war') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('Winning a War — PixelAnnex Endgame Guide', 'How a PixelAnnex war is actually won: the 65/65 bloc condition, why alliances count as one entity, what sudden death changes, and how conquest scoring rewards late-game aggression.',
      '<h1>Winning a war</h1><p>A war does not end when one player is bored. It ends on a specific, measurable condition, and knowing that condition changes how you should play the final stretch.</p><h2>The win condition</h2><p>A <strong>bloc</strong> wins when it controls at least 65% of the world\'s playable nations <em>and</em> at least 65% of its land. Both bars have to be cleared at once.</p><p>A bloc is either a single country and everything it holds, or an entire alliance counted as one entity. That second part is the important one: <strong>an alliance wins together</strong>. Two mid-sized powers coordinating will clear both bars long before either could alone, which makes diplomacy a genuine win path rather than a flavour mechanic.</p><p>The two-bar design is deliberate. Taking a great many tiny nations runs up your country count while barely moving your land share; swallowing one enormous country does the reverse. Winning requires both breadth and mass, so a late-game push usually has to be aimed at the bar you are furthest from, not simply at whatever is easiest to take.</p><h2>Sudden death</h2><p>When only a handful of nations are left standing, sudden death begins and <strong>every surviving nation\'s regeneration doubles</strong>.</p><p>This is the most volatile phase of the game and the point at which wars most often flip. A nation that looked finished is suddenly refilling twice as fast, and defenders benefit exactly as much as the leader does. If you are ahead, sudden death is when to press rather than consolidate; if you are behind, it is your best and possibly last window.</p><h2>Scoring rewards the endgame</h2><p>Painting a pixel scores one point. Encircling one scores two. <strong>Conquering a country scores between 1,000 and 20,000</strong>, scaled to how large it is.</p><p>The scale of that gap is easy to underestimate. A single large conquest can be worth more than an entire day of steady painting, which means that late in a war &mdash; when the remaining nations are the big ones &mdash; aggression pays enormously better than accumulation. Points determine your rank, and ranks run from Soldier through Lieutenant, Captain and General to Admiral, so the endgame is also where ranks are won.</p><p>Every country you personally conquer also permanently unlocks its <a href="/guides/the-living-world">collectable item</a>, which survives the reset.</p><h2>What carries over</h2><p>When the win condition is met, a winner is declared and the map resets for a new war. Your rank, your points and your collection persist across wars; the territory does not. Nothing on the map is permanent, which is rather the point &mdash; every war starts with all nations standing and every border up for grabs.</p>', '/guides/winning-a-war'));
    return;
  }
  if (url.pathname === '/guides/the-living-world') {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=3600' });
    res.end(_seoPage('The Living World — Bots, Monsters, Day/Night and Collectables', 'What runs in PixelAnnex while nobody is watching: 166 AI nations that fight on their own, roaming monsters that erase territory, a real-time day/night cycle, and 250 collectable items.',
      '<h1>The living world</h1><p>Most browser games are empty when you are not in them. PixelAnnex is not, and several of its systems exist specifically so that logging in tomorrow shows you a world that moved without you.</p><h2>166 nations that play themselves</h2><p>Every country nobody is playing is run by an ambient AI that paints, defends its homeland and runs campaigns of its own. There are 166 of them, and they are not decorative &mdash; they conquer each other, they besiege human players, and they will take your country if you leave it undefended.</p><p>Their intensity <strong>scales with scarcity</strong>. Early in a war, with the full roster standing, they are relatively calm. As nations fall and the field narrows, the survivors grow markedly more aggressive, so the back half of a war is dramatically more violent than the opening &mdash; by design, so that a thinning map accelerates rather than stagnates.</p><p>They are also focused rather than random: a bot picks a target and keeps hammering it until it falls, and they will not waste effort attacking nations that are already dead.</p><h2>Monsters</h2><p>At intervals, a UFO, a Kraken or a giant lizard crosses the map and erases pixels in its path. They are entirely indiscriminate &mdash; they will eat your territory and your enemy\'s with equal enthusiasm.</p><p>Treat them as weather rather than as an attack. There is no counterplay beyond not painting directly in front of one, and a monster tearing through a contested front line has decided more than one border dispute.</p><h2>Real-time day and night</h2><p>The map runs on actual time. The terminator &mdash; the line between day and night &mdash; tracks the real position of the sun, complete with seasonal tilt, and city lights glow across the dark side. The status bar shows the current UTC time.</p><p>This is mostly atmosphere, but it has one genuine tactical use: <strong>it shows you which parts of the world are asleep</strong>. Territory in the middle of its local night has the fewest active human defenders, and expanding into a sleeping timezone is one of the cheapest ways to gain ground.</p><h2>Chests and collectables</h2><p>Treasure chests appear on the map roughly ten times an hour. The first player to click one takes it outright, along with a lump of pixels and points. They are worth abandoning whatever you were doing for.</p><p>Every country in the game also has a unique piece of pixel art attached to it. Conquer that country <em>yourself</em> &mdash; it has to be your own stroke that takes it, not an ally\'s &mdash; and the item is permanently added to your collection. There are 250 to find, and they survive every world reset, which makes them the one form of progress that accumulates across the entire life of the game.</p><p>New here? Start with <a href="/how-to-play">how to play</a>, then <a href="/guides/encirclement">the encirclement guide</a>.</p>', '/guides/the-living-world'));
    return;
  }
